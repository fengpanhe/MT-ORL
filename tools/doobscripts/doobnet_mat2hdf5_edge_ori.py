...
def BSDS__augmentation(mat_path, h5_dir, img_src_dir, img_dst_dir, testfg=False):
    ...
    for idx in range(2):
        ...
        if testfg:
            # print filename
            # img_dst_filename = os.path.join(img_dst_dir, '{}.jpg'.format(img_src_filename))
            # img.save(img_dst_filename, quality=100)
            h5_filename = os.path.join(h5_dir, '{}.h5'.format(mat_id))
            with h5py.File(h5_filename, 'w') as f:
                f['label'] = label
...
def main():
    ...
    if (args.dataset == 'BSDSownership'):
        ...
        for mat_path in mat_list:
            BSDS__augmentation(mat_path, dst_label_dir, args.img_dir, dst_img_dir, testfg=True)
