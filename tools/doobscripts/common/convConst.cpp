...
void mexFunction( int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[] ) {
  ...
  //cout << mxGetDimensions(prhs[1])[0] << ' ' << mxGetDimensions(prhs[1])[1] << endl;
  //ns = (int*)mxGetDimensions(prhs[1]);
  ns[0] = mxGetDimensions(prhs[1])[0];
  ns[1] = mxGetDimensions(prhs[1])[1];
  d = (nDims == 3) ? ns[2] : 1;
  cout << ns[0] << ' ' << ns[1] << endl;
  m = (ns[0] < ns[1]) ? ns[0] : ns[1];
  cout << nDims << ' ' << id << ' ' << m << endl;
  ...
  cout << ms[0] << ' ' << ms [1] << ' ' << d << endl;
  B = (float*) mxMalloc(ms[0]*ms[1]*d);
